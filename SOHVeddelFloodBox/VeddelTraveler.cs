...
public class VeddelTraveler : HumanTraveler
{
    ...
    public Household Household
    {
        ...
        set
        {
            if (value.FamilyMemebers.Contains(this) == false)
    ...
    public override void Tick()
    {
        ...
        if (_layer.SpatialGraphMediatorLayer.Environment.NearestNode(this.Position).Attributes.TryGetValue("underwater", out _))
        {
        ...
        if (Household.Type == HouseholdType.WAITING)
        {
            return;
        }
        ...
        if (Household.Type == HouseholdType.WALKING)
        {
            Move();
        }
    }
    ...
    private void CheckFamilyMemberSeparated()
    {
        ...
        foreach (var target in Household.FamilyMemebers)
        {
            var distance = this.Position.DistanceInMTo(target.Position);
            if (distance >= FamilyDistanceThreshold)
            {
                Household.VoteFamilyMeetingPoint();
            }
        }
    }
